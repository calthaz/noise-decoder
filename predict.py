...
model = np.load("model-mfccs-no-res/cathy-7-5.npz")
...
def predict_chunk(volume_threshold, time_seconds):
    block = []
    b = 0
    html = ""
    for _ in range(int(time_seconds*collect.RATE/collect.CHUNK)): #do this for 10 seconds
        ...
        if(len(block)==collect.n_chunks_per_block):
            ...
            block = block[1:]
            #print(d.shape)
            ao = predict(d/(collect.RATE/4), wh, bh, wo, bo)
...
def predict_mfccs(volume_threshold, time_seconds):
    ...
    html = ""
    for _ in range(int(time_seconds*collect.RATE/collect.CHUNK)): #do this for 10 seconds
        data=np.frombuffer(stream.read(collect.CHUNK),dtype=np.float32)
        #print(b)
        #print(len(block))
        #print(np.square(data[0:10].astype(np.float32)))
        volume = np.sqrt(
        ...
        if(len(block)==collect.n_chunks_per_block):
            ...
            mfccs = librosa.feature.mfcc(y=d, 
                sr=collect.RATE, n_mfcc=process.n_mfcc)#=d.astype(np.float32)
